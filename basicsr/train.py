...
def main():
    ...
    if resume_state:  # resume training
        ...
        # print(resume_state['optimizers'][0])
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            lq = train_data['lq']
            gt = train_data['gt']
            if opt['datasets']['train']['use_ueatten']:
                lh = train_data['lh']
            if opt['datasets']['train']['use_multask']:
                lh = train_data['lh']
            if opt['datasets']['train']['use_grayatten']:
                atten = train_data['gray']
                lh = train_data['lh']
                # atten = train_data['lh']
            if opt['datasets']['train']['use_illguidance']:
                atten = train_data['gray']
            ...
            if opt['datasets']['train']['use_grayatten']:
                model.feed_train_data({'lq': lq, 'gt':gt, 'lh':lh, 'gray':atten})  
                # model.feed_train_data({'lq': lq, 'gt':gt, 'gray':atten})
            elif opt['datasets']['train']['use_ueatten']:
                model.feed_train_data({'lq': lq, 'gt':gt, 'lh':lh})
            elif opt['datasets']['train']['use_multask']:
                model.feed_train_data({'lq': lq, 'gt':gt, 'lh':lh})
            elif opt['datasets']['train']['use_illguidance']:
                model.feed_train_data({'lq': lq, 'gt':gt, 'gray':atten})
            else:
